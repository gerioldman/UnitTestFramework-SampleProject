...
static void Grahics_CircleHelperFunction(int16_t xc, int16_t yc, int16_t x, int16_t y);
...
void Graphics_Char(char c,int16_t x,int16_t y)
{
...
void Graphics_String(char *string,int16_t x,int16_t y)
{
	...
	while(c != '\0')
	{
		Graphics_Char(c,x+char_offset,y);
		char_offset+=8;
		c = *string;
		string++;
	}
...
void Graphics_Circle(int16_t xc,int16_t yc,int16_t r)
{
	int x = 0, y = r;
	int d = 3 - 2 * r;
	Grahics_CircleHelperFunction(xc, yc, x, y);
	while (y >= x) {
		x++;
		if (d > 0) {
			y--;
